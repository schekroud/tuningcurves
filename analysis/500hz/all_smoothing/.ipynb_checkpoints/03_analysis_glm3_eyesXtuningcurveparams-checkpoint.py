...
for sub in subs:
    for fittype in ['opt', 'glm']:
        ...
        np.save(op.join(wd, 'data', 'glms', '500hz', 'all_smoothing', 'eye_glm', f'wmc_s{sub:02d}_glm3_betas_{fittext}_{addtext}.npy'), arr = allb)
        np.save(op.join(wd, 'data', 'glms', '500hz', 'all_smoothing', 'eye_glm', f'wmc_s{sub:02d}_glm3_tvalues_{fittext}_{addtext}.npy'), arr = allt)
        if sub == 4:
            np.save(op.join(wd, 'data', 'glms', '500hz', 'all_smoothing', 'eye_glm', 'eyetracker_times.npy'), arr = data.times)
            np.save(op.join(wd, 'data', 'glms', '500hz', 'all_smoothing', 'eye_glm', f'regressor_names.npy'), arr = model.regressor_names)