# -*- coding: utf-8 -*-
"""
Created on Wed Sep 25 23:32:57 2024
...
smooth_alphas = True
smooth_distances = True #smooth distances before parameter estimation
smoothsig = 10
...
for i in subs:
    ...
    if op.exists(op.join(wd, 'data', 'tuningcurves', '500hz', 'parameter_fits', 'all_smoothing',
                         f's{i}_ParamFits_Alpha_binstep{binstep}_binwidth{binwidth}.npy')):
        print(f'\n- -  loading in previously estimated tuning curve precisions')
        alphas = np.load(op.join(wd, 'data', 'tuningcurves', '500hz', 'parameter_fits', 'all_smoothing',
                f's{i}_ParamFits_Alpha_binstep{binstep}_binwidth{binwidth}.npy'))
    else:
    ...
    if i==4:
        ...
        np.save(op.join(wd, 'data', 'tuningcurves', '500hz', 'parameter_fits', 'all_smoothing', 'model_settings.npy'), model_settings)
    
    #save tuning curve model parameters
    np.save(op.join(wd, 'data', 'tuningcurves', '500hz', 'parameter_fits', 'all_smoothing',
