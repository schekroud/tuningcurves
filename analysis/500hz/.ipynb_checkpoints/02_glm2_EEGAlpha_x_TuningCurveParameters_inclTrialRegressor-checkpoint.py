...
for sub in subs:
    ...
    for fittype in ['opt', 'glm']:
        ...
        if fittype == 'glm':
            amp = np.load(op.join(wd, 'data', 'tuningcurves', '500hz', 'parameter_fits', 'twostage_alphaminmaxfit',
        ...
        np.save(op.join(wd, 'data', 'glms', '500hz', 'eeg_glm', f'wmc_s{sub:02d}_glm2{logtxt}_betas_{fittext}_{addtext}.npy'), arr = allb)
        np.save(op.join(wd, 'data', 'glms', '500hz', 'eeg_glm', f'wmc_s{sub:02d}_glm2{logtxt}_tvalues_{fittext}_{addtext}.npy'), arr = allt)
        if sub == 4:
            np.save(op.join(wd, 'data', 'glms', '500hz', 'eeg_glm', 'eeg_times.npy'), arr = eegtimes)
            np.save(op.join(wd, 'data', 'glms', '500hz', 'eeg_glm', f'regressor_names.npy'), arr = model.regressor_names)