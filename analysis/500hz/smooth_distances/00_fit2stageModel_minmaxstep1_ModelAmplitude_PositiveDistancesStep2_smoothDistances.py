# -*- coding: utf-8 -*-
"""
Created on Tue Oct  1 10:11:15 2024
...
for i in subs:
    ...
    if smooth_distances:
        data = sp.ndimage.gaussian_filter1d(data, sigma = distsmooth_sigma) #smooth with a 10sd (20ms at 500hz) gaussian blur
    ...
    if op.exists(op.join(wd, 'data', 'tuningcurves', 'parameter_fits', '500hz', 'smooth_distances',
                         f's{i}_ParamFits_precision_binstep{binstep}_binwidth{binwidth}.npy')):
        print(f'\n- -  loading in previously estimated tuning curve precisions')
        alphas = np.load(op.join(wd, 'data', 'tuningcurves', 'parameter_fits', '500hz', 'smooth_distances',
                f's{i}_ParamFits_precision_binstep{binstep}_binwidth{binwidth}.npy'))
        alphas = alphas[:,:,0,:].copy()
    else:
        ...
        toc1=time.time()
        print(f'- - tuning curve precision modelling took {int(divmod(toc1-tic1, 60)[0])}m{round(divmod(toc1-tic1, 60)[1])}s')
    

        alphas = step1params[:,:,0,:].copy() #get just the fitted alpha values, we dont care about b1 here
        #we dont want to smooth precision as we are smoothing distances before fitting, and constraining in the amplitude fitting step
    ...
    toc2=time.time()
    print(f'- - amplitude modelling took {int(divmod(toc2-tic2, 60)[0])}m{round(divmod(toc2-tic2, 60)[1])}s')
    ...
    if i==4:
        ...
        model_settings['smooth_sigma'] = distsmooth_sigma
        model_settings['constrain_alpha'] = constrain_alpha
        model_settings['smooth_distances'] = smooth_distances
        model_settings['distance_smooth'] = distsmooth_sigma 
        model_settings['minmax_stage1'] = True
        model_settings['minmax_designmatrix_stage2'] = True
        
        np.save(op.join(wd, 'data', 'tuningcurves', 'parameter_fits', 'twostageminmax_smoothdists', 'model_settings.npy'), model_settings)
    
    #save tuning curve model parameters
    np.save(op.join(wd, 'data', 'tuningcurves', '500hz', 'parameter_fits', 'smooth_distances',
