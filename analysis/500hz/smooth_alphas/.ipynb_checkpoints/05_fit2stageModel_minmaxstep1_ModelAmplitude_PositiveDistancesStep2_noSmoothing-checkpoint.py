# -*- coding: utf-8 -*-
"""
Created on Wed Sep 25 12:57:09 2024
...
for i in subs:
    ...
    if op.exists(op.join(wd, 'data', 'tuningcurves', '500hz', 'parameter_fits', 'smooth_alphas',
                         f's{i}_ParamFits_Alpha_binstep{binstep}_binwidth{binwidth}.npy')):
        print(f'\n- -  loading in previously estimated tuning curve precisions')
        alphas = np.load(op.join(wd, 'data', 'tuningcurves', '500hz', 'parameter_fits', 'smooth_alphas',
                f's{i}_ParamFits_Alpha_binstep{binstep}_binwidth{binwidth}.npy'))
    else:
    ...
    if i==4:
        ...
        np.save(op.join(wd, 'data', 'tuningcurves', '500hz', 'parameter_fits', 'smooth_alphas', 'model_settings.npy'), model_settings)
    
    #save tuning curve model parameters
    np.save(op.join(wd, 'data', 'tuningcurves', '500hz', 'parameter_fits', 'smooth_alphas',
