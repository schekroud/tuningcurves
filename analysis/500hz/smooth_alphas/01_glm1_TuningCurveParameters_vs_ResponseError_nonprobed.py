...
for sub in subs:
    ...
    ampglm = np.load(op.join(wd, 'data', 'tuningcurves', '500hz', 'parameter_fits', 'smooth_alphas',
    ...
    #get precision and amplitude of the item that was NOT probed
    nonpitem = np.where(pitem==0, 1, 0) #invert to get the index of the non probed item on a given trial
    itemprec = np.zeros(shape = [ntrials, ntimes]) * np.nan
    itemamp = np.zeros(shape = [ntrials, ntimes]) * np.nan
    for itrl in range(ntrials):
        ipside = nonpitem[itrl]
...
np.save(op.join(wd, 'data', 'glms', '500hz', 'smooth_alphas', 'beh_glm', f'TCParamsXbehaviour_beta_precision_binstep{binstep}_binwidth{binwidth}_nonprobed.npy'),  arr = b_prec)
np.save(op.join(wd, 'data', 'glms', '500hz', 'smooth_alphas', 'beh_glm', f'TCParamsXbehaviour_beta_amplitude_binstep{binstep}_binwidth{binwidth}_nonprobed.npy'),  arr = b_amp)
np.save(op.join(wd, 'data', 'glms', '500hz', 'smooth_alphas', 'beh_glm', f'TCParamsXbehaviour_tstat_precision_binstep{binstep}_binwidth{binwidth}_nonprobed.npy'), arr = t_prec)
np.save(op.join(wd, 'data', 'glms', '500hz', 'smooth_alphas', 'beh_glm', f'TCParamsXbehaviour_tstat_amplitude_binstep{binstep}_binwidth{binwidth}_nonprobed.npy'), arr = t_amp)
