# -*- coding: utf-8 -*-
"""
Created on Fri Sep  6 14:50:58 2024
...
for sub in subs:
    for fittype in ['opt', 'glm']:
        ...
        np.save(op.join(wd, 'data', 'glms', 'eeg', 'glm1', f'wmc_s{sub:02d}_glm1{logtxt}_betas_{fittext}_{addtext}{amptxt}.npy'), arr = allb)
        np.save(op.join(wd, 'data', 'glms', 'eeg', 'glm1', f'wmc_s{sub:02d}_glm1{logtxt}_tvalues_{fittext}_{addtext}{amptxt}.npy'), arr = allt)
        if sub == 4:
            np.save(op.join(wd, 'data', 'glms', 'eeg', 'glm1', 'eeg_times.npy'), arr = eegtimes)
            np.save(op.join(wd, 'data', 'glms', 'eeg', 'glm1', f'regressor_names.npy'), arr = model.regressor_names)
