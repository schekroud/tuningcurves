...
modeltimes = modeltimes[1:] #we're looking at the first derivative here

nparams=2 #how many parameters will we fit in later models

b_precd = np.zeros(shape = [nsubs, nparams, modeltimes.size]) * np.nan
t_precd = b_precd.copy()
b_ampd  = b_precd.copy()
t_ampd  = b_precd.copy()
...
for sub in subs:
    ...
    b_precd[subcount] = betas_prec
    b_ampd[subcount]  = betas_amp
    t_precd[subcount] = tvals_prec
    t_ampd[subcount]  = tvals_amp
    
np.save(op.join(wd, 'data', 'glms', 'tc_behaviour', 'glm1', 'firstderiv_TCParamsXbehaviour_beta_precision.npy'),  arr = b_precd)
np.save(op.join(wd, 'data', 'glms', 'tc_behaviour', 'glm1', 'firstderiv_TCParamsXbehaviour_beta_amplitude.npy'),  arr = b_ampd)
np.save(op.join(wd, 'data', 'glms', 'tc_behaviour', 'glm1', 'firstderiv_TCParamsXbehaviour_tstat_precision.npy'), arr = t_precd)
np.save(op.join(wd, 'data', 'glms', 'tc_behaviour', 'glm1', 'firstderiv_TCParamsXbehaviour_tstat_amplitude.npy'), arr = t_ampd)
