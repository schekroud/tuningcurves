# -*- coding: utf-8 -*-
"""
Created on Fri Sep 20 01:09:46 2024
...
for i in subs:
    ...
    if smooth_distances:
        data = sp.ndimage.gaussian_filter1d(data, sigma = 3) #smooth with a 3sd (30ms as 100hz) gaussian blur
    ...
    if op.exists(op.join(wd, 'data', 'tuningcurves', 'parameter_fits', 'twostageminmax_smoothdists',
                         f's{i}_ParamFits_precision_binstep{binstep}_binwidth{binwidth}.npy')):
        print(f'\n- -  loading in previously estimated tuning curve precisions')
        alphas = np.load(op.join(wd, 'data', 'tuningcurves', 'parameter_fits', 'twostageminmax_smoothdists',
                f's{i}_ParamFits_precision_binstep{binstep}_binwidth{binwidth}.npy'))
    else:
            ...
            for itrl in range(ntrials):
                ...
                for tp in range(ntimes):
                    ...
                    res = sp.optimize.curve_fit(lambda x, B1, alpha: tcf.fullCosineModel(x, 0, B1, alpha), #fixes B0 at 0 (removes from model, keeps everything else)
                                                      ...
                                                      bounds = ([-np.inf, 0.001], [np.inf, 3]), #b1 unbounded, alpa between 0.001 and 3
        ...
        #we dont need to smooth precision, or constrain it, as we are smoothing distances before fitting, and constraining in the estimation
    ...
    if i==4:
        ...
        model_settings = dict() 
        model_settings['smooth_alphas'] = False
        model_settings['smooth_sigma'] = None
        model_settings['constrain_alpha'] = False
        model_settings['smooth_distances'] = True
        model_settings['distance_smooth'] = 3 #distance smoothed by 3sd blur (30ms)
    ...
    np.save(op.join(wd, 'data', 'tuningcurves', 'parameter_fits', 'twostageminmax_smoothdists',
